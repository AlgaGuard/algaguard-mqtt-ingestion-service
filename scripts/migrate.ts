...
try {
  await client.query(
    "SELECT pg_advisory_lock(hashtext('algaguard-mqtt-ingestion-migrations'))",
  );
  await client.query(
  ...
  for (const filename of (await readdir(path.resolve("migrations")))
    ...
    const prior = await client.query(
      ...
      ["algaguard-mqtt-ingestion-service", filename],
    );
    if (prior.rows[0]) {
    ...
    try {
      ...
      await client.query(
        ...
        ["algaguard-mqtt-ingestion-service", filename, sha],
      );
  ...
  await client.query(
    "SELECT pg_advisory_unlock(hashtext('algaguard-mqtt-ingestion-migrations'))",
  );
  client.release();
  await pool.end();
}
